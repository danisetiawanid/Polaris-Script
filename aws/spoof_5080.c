// ====================================================
// libnvml_fake.c (FULL SPOOF NVIDIA RTX 5080, VRAM 26 GB)
// ====================================================
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
// === Device info ===
nvmlReturn_t nvmlDeviceGetName(nvmlDevice_t d,char* n,unsigned int l){ strncpy(n,"NVIDIA RTX 5080",l); return NVML_SUCCESS; }
nvmlReturn_t nvmlDeviceGetUUID(nvmlDevice_t d,char* u,unsigned int l){ strncpy(u,"GPU-RTX5080-UUID",l); return NVML_SUCCESS; }
nvmlReturn_t nvmlDeviceGetSerial(nvmlDevice_t d,char* s,unsigned int l){ strncpy(s,"5080-1234567890",l); return NVML_SUCCESS; }

// === Memory Info (26 GB spoof) ===
nvmlReturn_t nvmlDeviceGetMemoryInfo(nvmlDevice_t d,nvmlMemory_t* m){
    if (!m) return NVML_SUCCESS;
    m->total = 30ULL << 30;   // 30 GB
...
#define RTX4090_CUDA_CORES   16384
#define RTX4090_TENSOR_CORES 612
...
nvmlReturn_t nvmlDeviceGetAttributes_v2(nvmlDevice_t d, nvmlDeviceAttributes_t *attr) {
    ...
    attr->memorySizeMB        = 30624;            // 26 GB dalam MB
    return NVML_SUCCESS;
}
