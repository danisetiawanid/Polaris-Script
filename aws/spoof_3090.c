// ====================================================
// libnvml_fake.c (FULL SPOOF NVIDIA RTX 3090, VRAM 26 GB)
// ====================================================
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
// === Device info ===
nvmlReturn_t nvmlDeviceGetName(nvmlDevice_t d,char* n,unsigned int l){ strncpy(n,"NVIDIA RTX 3090",l); return NVML_SUCCESS; }
nvmlReturn_t nvmlDeviceGetUUID(nvmlDevice_t d,char* u,unsigned int l){ strncpy(u,"GPU-RTX3090-UUID",l); return NVML_SUCCESS; }
nvmlReturn_t nvmlDeviceGetSerial(nvmlDevice_t d,char* s,unsigned int l){ strncpy(s,"3090-1234567890",l); return NVML_SUCCESS; }

// === Memory Info (26 GB spoof) ===
nvmlReturn_t nvmlDeviceGetMemoryInfo(nvmlDevice_t d,nvmlMemory_t* m){
    if (!m) return NVML_SUCCESS;
    m->total = 28ULL << 30;   // 26 GB
...
nvmlReturn_t nvmlDeviceGetAttributes_v2(nvmlDevice_t d, nvmlDeviceAttributes_t *attr) {
    ...
    attr->memorySizeMB        = 26624;            // 26 GB dalam MB
    return NVML_SUCCESS;
}
