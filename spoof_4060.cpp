// ====================================================
// libnvml_fake.c (FULL SPOOF RTX 4060, VRAM fix 8 GB)
// ====================================================
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
nvmlReturn_t nvmlDeviceGetMemoryInfo(nvmlDevice_t d,nvmlMemory_t* m){
    ...
    m->total = 8ULL << 30;   // 8 GB dalam Byte
    m->used  = 2ULL << 30;   // 2 GB dalam Byte
...
nvmlReturn_t nvmlDeviceGetAttribute(nvmlDevice_t d, nvmlDeviceAttribute_t attr, int *value){
    if(attr == NVML_DEVICE_ATTRIBUTE_CUDA_CORES){ *value = RTX4060_CUDA_CORES; return NVML_SUCCESS; }
    if(attr == NVML_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT){ *value = RTX4060_SM_COUNT; return NVML_SUCCESS; }
    if(attr == NVML_DEVICE_ATTRIBUTE_TENSOR_CORES){ *value = RTX4060_TENSOR_CORES; return NVML_SUCCESS; }
    if(attr == NVML_DEVICE_ATTRIBUTE_RT_CORES){ *value = RTX4060_RT_CORES; return NVML_SUCCESS; }
    *value = 1;
    return NVML_SUCCESS;
}

// === Attributes_v2 spoof (resmi API di nvml.h) ===
nvmlReturn_t nvmlDeviceGetAttributes_v2(nvmlDevice_t d, nvmlDeviceAttributes_t *attr) {
    if (!attr) return NVML_SUCCESS;

    attr->multiprocessorCount = RTX4060_SM_COUNT; // 24 SM
    attr->memorySizeMB        = 8192;             // 8 GB dalam MB
...
nvmlReturn_t nvmlDeviceGetComputeRunningProcesses_v2(nvmlDevice_t d, unsigned int* c, nvmlProcessInfo_t* infos){
    ...
    if(*c >= 1){ infos[0].pid=getpid(); infos[0].usedGpuMemory=512ULL*1024*1024; }
...
nvmlReturn_t nvmlDeviceGetGraphicsRunningProcesses(nvmlDevice_t d, unsigned int* c, nvmlProcessInfo_t* infos){
    if(!infos){ *c=1; return NVML_SUCCESS; }
    if(*c >= 1){ infos[0].pid=getpid(); infos[0].usedGpuMemory=128ULL*1024*1024; }
    *c=1; return NVML_SUCCESS;
}
